...
public class CityGiftItemViewModel : BaseViewModel
{
    ...
    public string AppliedMessage
    {
        ...
        set
        {
            SetProperty(ref _appliedMessage, value);
            OnPropertyChanged(nameof(HasAppliedMessage));
        }
    }
...
public class CityGiftsViewModel : BaseViewModel
{
    ...
    public (bool success, string? error) Apply(long cityGiftId)
    {
        ...
        if (result.success)
        {
            LoadGifts(SelectedNeighborhood.Id);
            var gift = Gifts.FirstOrDefault(g => g.Id == cityGiftId);
            if (gift != null)
                gift.AppliedMessage = "Successfully applied!";
        }
        return result;
    }
